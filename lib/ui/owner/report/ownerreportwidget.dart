...
class ReportPersonHeaderCard extends StatelessWidget {
  ...
  // FIX: previously this crashed with RangeError(length: 0) whenever
  // `name` was an empty string. `''.split(RegExp(r'\s+'))` returns
  // `['']` (a list with ONE empty string), not an empty list — so
  // `parts.isEmpty` was false and `parts.first.substring(0, 1)` ran
  // on an empty string, throwing "Invalid value: Only valid value is 0: 1".
  // Now we trim, drop empty parts, and fall back to '?' when there's
  // nothing usable.
  String get _initials {
