...
//         },
//         builder: (context, state) {
//           final isSubmitting = state.submitStatus == OwnerDespatchSubmitStatus.inProgress;
//
//           return PopScope(
//             // Block back button / gesture entirely while a request is
//             // in flight, so it can never race with the success-listener
//             // pop above and cause a double-pop on go_router's stack.
//             canPop: !isSubmitting,
//             onPopInvokedWithResult: (didPop, result) {
//               if (!didPop && isSubmitting) {
//                 AppSnackbar.error('Please wait until the despatch sheet finishes saving.');
//               }
//             },
//             child: Scaffold(
//               backgroundColor: AppColors.background,
//               appBar: AppBar(
//                 title: Text('Create Despatch Sheet', style: AppTextStyles.h6()),
    ...
    return NetworkAwareWrapper(
      child: BlocConsumer<OwnerDespatchSheetBloc, OwnerDespatchSheetState>(
        ...
        listener: (context, state) {
          if (state.submitStatus == OwnerDespatchSubmitStatus.success) {
            AppSnackbar.success(state.submitMessage ?? 'Despatch sheet created');
            // Guard: only pop if this screen is still on top AND there is
            // something to pop back to. This is the SINGLE place this
            // screen is ever popped from now — the back button is fully
            // blocked below while a submit is in progress, so this can
            // never race with a manual back-press.
            if (context.mounted && Navigator.of(context).canPop()) {
              Navigator.of(context).pop(true);
            }
          } else if (state.submitStatus == OwnerDespatchSubmitStatus.failure) {
        ...
        builder: (context, state) {
          final isSubmitting =
              state.submitStatus == OwnerDespatchSubmitStatus.inProgress;

          return PopScope(
            // Blocks the AppBar back arrow, the system back button, AND
            // the edge swipe gesture while a request is in flight. This
            // is what actually prevents the double-pop crash you were
            // seeing — previously the back arrow stayed tappable even
            // while "Please wait..." was showing.
            canPop: !isSubmitting,
            onPopInvokedWithResult: (didPop, result) {
              if (!didPop && isSubmitting) {
                AppSnackbar.error(
                    'Please wait until the despatch sheet finishes saving.');
              }
            },
            child: Scaffold(
              backgroundColor: AppColors.background,
              appBar: AppBar(
                title: Text('Create Despatch Sheet', style: AppTextStyles.h6()),
                // Hides/disables the back arrow visually too, so it's not
                // just blocked but also not shown as tappable.
                automaticallyImplyLeading: !isSubmitting,
              ),
              body: SafeArea(
                child: Builder(
