...
//   bool _dropdownSelectionSynced = false;
//
//   // True when the currently-selected unit is a "box" unit. Controls whether
//   // the packing / pieces-per-box fields show up and get sent to the API —
//   // per the API's "only if boxes" requirement. Adjust the check inside
//   // _computeIsBoxUnit if "box" isn't determined by the unit's label in your
//   // real unit data.
...
//   }
//
//   /// Adjust this if "box" isn't determined by the unit's label in your data.
...
//     return matches.first.label.toLowerCase().contains('box');
//   }
//
...
//                   SizedBox(height: Responsive.h(14)),
//
//                   // Packing / pieces-per-box — only shown for box-type units.
//                   if (_isBoxUnit) ...[
//                     Row(
//                       crossAxisAlignment: CrossAxisAlignment.start,
//                       children: [
...
class _AddIncentiveProductFormState extends State<_AddIncentiveProductForm> {
  ...
  // True when the currently-selected unit is "Box" OR "Square Feet".
  // Controls whether the packing / pieces-per-box fields show up and get
  // sent to the API. Tiles are sold by SqFt but are still packed in boxes,
  // so both unit types need these fields — not just "Box".
  bool _isBoxUnit = false;
  ...
  /// Returns true when the selected unit is "Box" or "Square Feet".
  /// Tiles are sold by SqFt but still packed/counted in boxes, so both
  /// unit types need the Packing / Pieces-per-Box fields — not just "Box".
  ///
  /// Matches on normalized (lowercased, spaces stripped) name AND
  /// abbreviation so small formatting differences from the API (e.g.
  /// "Sqare Feets - SqoFt") don't silently break this check. If the real
  /// /units/active response uses different spellings than the variants
  /// listed below, add them here.
  bool _computeIsBoxUnit(ProductState state) {
    if (_selectedUnitId == null) return false;
    final matches = state.units.where((u) => u.id == _selectedUnitId);
    if (matches.isEmpty) return false;
    ...
    final name = unit.name.toLowerCase().replaceAll(' ', '');
    final abbr = unit.abbreviation.toLowerCase().replaceAll(' ', '');

    const boxNames = ['box'];
    const boxAbbrs = ['box'];
    const sqftNames = ['squarefeet', 'sqarefeets', 'squarefeets'];
    const sqftAbbrs = ['sqft', 'sqoft'];

    final isBox = boxNames.contains(name) || boxAbbrs.contains(abbr);
    final isSqft = sqftNames.contains(name) || sqftAbbrs.contains(abbr);
            ...
            return Form(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  // Packing / pieces-per-box — only shown for Box or
                  // Square Feet units.
                  if (_isBoxUnit) ...[
                    Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
