...
class _AddIncentiveProductFormState extends State<_AddIncentiveProductForm> {
  ...
  // True when the currently-selected unit's API record has
  // show_pieces_per_box = "1". Controls whether the Pieces per Box field
  // shows up / gets auto-filled / gets sent to the API. Packing itself is
  // now shown for every unit regardless of this flag.
  bool _isBoxUnit = false;

  // Guards against feedback loops when we programmatically set
  // _piecesPerBoxCtrl.text from inside the packing listener.
  bool _syncingPiecesFromPacking = false;
  ...
  void initState() {
    ...
    // box/sq-ft type unit that actually uses pieces-per-box.
    _packingCtrl.addListener(_onPackingChanged);
  ...
  void dispose() {
    ...
    _minQuantityCtrl.dispose();
    _piecesPerBoxCtrl.dispose();
    _packingCtrl.removeListener(_onPackingChanged);
    _packingCtrl.dispose();
    super.dispose();
  }

  // Pulls the first number out of the Packing text and mirrors it into
  // Pieces per Box, e.g. "8pcs/box" -> "8". Only runs for box/sq-ft type
  // units; for other units Pieces per Box stays untouched (and hidden).
  void _onPackingChanged() {
    if (_syncingPiecesFromPacking) return;
    if (!_isBoxUnit) return;

    final match = RegExp(r'\d+').firstMatch(_packingCtrl.text);
    final extracted = match?.group(0) ?? '';

    if (_piecesPerBoxCtrl.text != extracted) {
      _syncingPiecesFromPacking = true;
      _piecesPerBoxCtrl.text = extracted;
      _syncingPiecesFromPacking = false;
    }
  }
  ...
  void _onUnitChanged(String? unitId, ProductState state) {
    setState(() {
      ...
      _isBoxUnit = _computeIsBoxUnit(state);
      if (!_isBoxUnit) {
        // Packing stays visible/editable for every unit now, so it's left
        // alone here. Only Pieces per Box is unit-gated, so clear it when
        // the newly selected unit doesn't use it.
        _piecesPerBoxCtrl.clear();
      } else {
        // Switched into a box/sq-ft unit — immediately derive Pieces per
        // Box from whatever is already typed in Packing.
        _onPackingChanged();
      }
    });
  }
  ...
  void _save(BuildContext context) {
    ...
    final bloc = context.read<ProductBloc>();
    final packingValue =
    _packingCtrl.text.trim().isEmpty ? null : _packingCtrl.text.trim();
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              TextFormField(
                                ...
                                inputFormatters: DValidator.digitsOnly,
                                decoration: const InputDecoration(hintText: 'e.g. 8'),
                                // Auto-filled from Packing, but still
                                // editable in case the auto-extracted
                                // number needs a manual correction.
                              ),
                            ],
                          ),
                        ),
