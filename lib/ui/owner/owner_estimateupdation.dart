...
// /// notes, terms, date, handling charge and item list via
// /// POST /estimates/update. Expects to be pushed with a
...
//
//   /// Handling charge for the estimate. Pre-filled from
//   /// [EstimateDetailModel.handlingCharge]; left blank means "leave
//   /// unchanged" on submit (see [OwnerUpdateEstimateRequest.handlingCharge]).
...
//   }
//
// // NEW: mirrors _validateContractorName in the other direction — once a
// // contractor name is entered, a contractor phone becomes required.
...
//       }
//       return null; // both empty — contractor section is optional
//     }
//     // Phone was entered — still validate its format.
...
//       // Blank field => leave the server's existing handling charge
//       // untouched (see OwnerUpdateEstimateRequest.handlingCharge).
...
//                         // NEW: re-run form validation as the name changes, so the phone
//                         // field's "required once name is present" rule updates immediately.
...
//                         inputFormatters: DValidator.phoneNumber,
//                         // CHANGED: was _validateOptionalPhone — now uses the mutual-required check.
...
//                             validator: (v) => DValidator.validateOptionalNumber('Handling charge', v),
//                           ),
//                         ),
//                         LabeledField(
...
// contractor name is entered, a contractor phone becomes required.
  ...
  /// Populates the Add/Edit Item form above with this saved row's data
  /// and flips it into "edit mode" (see _editingItemIndex). Triggered by
  /// tapping the pencil icon on a row in the Items list below. The actual
  /// save-back-to-list happens when the user taps "Update Item" in the
  /// form, which recalculates the amount via _saveItemFromForm() — the
  /// server call for the whole estimate only happens later, when "Save
  /// Changes" is tapped.
  void _editItem(int index) {
    final item = _items[index];
    final match = _findProduct(item.productId);
    ...
    // Scroll intent: the form lives above the list, so let the user know
    // where to look. A ScrollController could auto-scroll to the top if
    // desired — not added here to keep this change minimal.
