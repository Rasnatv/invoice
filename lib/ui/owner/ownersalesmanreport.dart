...
// // ---------------- EXCEL EXPORT HELPER ----------------
//
// /// Builds an .xlsx workbook for the given list of documents and shares it
// /// via the platform share sheet (Print / Save to Files / WhatsApp / etc.),
// /// so the user can open it in Excel and print it.
// Future<void> _exportDocumentsToExcel({
//   required BuildContext context,
//   required _DocType type,
//   required _PersonOption person,
...
//   }
//
//   final isBill = type == _DocType.bill;
//   final dateFmt = DateFormat('dd-MM-yyyy');
//   final sheetName = isBill ? 'Bills' : 'Quotations';
//
//   final workbook = xls.Excel.createExcel();
//   final xls.Sheet sheet = workbook[sheetName];
//   // Default sheet created by the package is named 'Sheet1'; drop it once
//   // our named sheet has content so only one tab is left.
//   if (workbook.sheets.containsKey('Sheet1') && sheetName != 'Sheet1') {
//     workbook.delete('Sheet1');
//   }
//
//   xls.CellStyle headerStyle = xls.CellStyle(bold: true);
//
//   // Title / filter context rows.
//   sheet.appendRow([xls.TextCellValue('${isBill ? 'Bills' : 'Quotations'} Report')]);
//   sheet.appendRow([
//     xls.TextCellValue('Salesman'),
//     xls.TextCellValue('${person.name} (${person.mobile})'),
//   ]);
//   sheet.appendRow([
//     xls.TextCellValue('Period'),
//     xls.TextCellValue('${dateFmt.format(fromDate)} to ${dateFmt.format(toDate)}'),
//   ]);
//   sheet.appendRow([]); // spacer row
//
//   final headers = <String>[
//     'Sl.No',
//     isBill ? 'Bill No' : 'Quotation No',
//     'Date',
//     'Customer Name',
//     'Mobile',
//     'Address',
...
//       xls.TextCellValue(d.id),
//       xls.TextCellValue(dateFmt.format(d.date)),
//       xls.TextCellValue(d.customerName),
//       xls.TextCellValue(d.customerMobile),
//       xls.TextCellValue(d.customerAddress),
//       xls.DoubleCellValue(d.sqft),
//       xls.DoubleCellValue(d.amount),
//       if (isBill) xls.TextCellValue(d.approvedBy ?? '-'),
//     ];
//     sheet.appendRow(row);
//   }
//
...
//     xls.DoubleCellValue(documents.fold(0.0, (s, d) => s + d.sqft)),
//     xls.DoubleCellValue(total),
//   ];
//   sheet.appendRow(totalRow);
//
//   // Reasonable column widths so it prints cleanly.
//   final widths = [6.0, 14.0, 12.0, 20.0, 14.0, 26.0, 10.0, 12.0, 14.0];
...
//     if (context.mounted) {
//       ScaffoldMessenger.of(context).showSnackBar(
//         const SnackBar(content: Text('Failed to build the Excel file')),
//       );
//     }
//     return;
//   }
//
//   final safeName = person.name.replaceAll(RegExp(r'[^A-Za-z0-9]+'), '_');
//   final fileName =
//       '${isBill ? 'Bills' : 'Quotations'}_${safeName}_${dateFmt.format(fromDate)}_${dateFmt.format(toDate)}.xlsx';
//
...
//     subject: '${isBill ? 'Bills' : 'Quotations'} Report - ${person.name}',
//     text: '${dateFmt.format(fromDate)} to ${dateFmt.format(toDate)}',
//   );
// }
//
...
// /// [_DocumentDetailScreen]. An "Export to Excel" action in the app bar
// /// exports the currently filtered rows so they can be printed from Excel.
...
//   }
//
//   Future<void> _exportSingleToExcel(BuildContext context) async {
//     await _exportDocumentsToExcel(
//       context: context,
//       type: doc.type,
//       person: person,
...
//                     onPressed: () => _exportSingleToExcel(context),
//                   ),
//                   SizedBox(width: Responsive.w(10)),
//                   _RoundIconButton(
