...
/// Change Password screen for the Driver flow.
/// Uses the SAME ProfileBloc / ProfileEvent / ProfileState / ProfileProvider
/// as the Owner screen — the API and password rules are shared across
/// owner / salesman / driver logins, only the presentation differs slightly
/// to match the driver dashboard's rounded, card-based look.
class DriverChangePasswordScreen extends StatelessWidget {
  const DriverChangePasswordScreen({super.key});

  @override
  Widget build(BuildContext context) {
    // Provide a fresh ProfileBloc here if one isn't already above this route
    // in the tree. If the driver dashboard already wraps things in a
    // ProfileBloc provider, you can drop this BlocProvider and just use
    // `const _DriverChangePasswordView()` directly.
    return BlocProvider(
      create: (_) => ProfileBloc(),
      child: const _DriverChangePasswordView(),
    );
  }
}

class _DriverChangePasswordView extends StatefulWidget {
  const _DriverChangePasswordView();

  @override
  State<_DriverChangePasswordView> createState() => _DriverChangePasswordViewState();
}

class _DriverChangePasswordViewState extends State<_DriverChangePasswordView> {
