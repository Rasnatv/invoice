...
/// independently of the whole-quotation POST /quotations/update. Used
/// when the owner taps "Update Item" on an item that's already saved on
/// the server (a real backend id, not one of the screen's own local
/// 'new_' ids) — so the change survives immediately instead of only
/// living in local state until "Save Changes" is tapped.
class OwnerQuotationItemUpdateSubmitted extends OwnerQuotationEditEvent {
  ...
  const OwnerQuotationItemUpdateSubmitted({
  ...
  @override
  List<Object?> get props =>
      [quotationId, quotationItemId, quantity, rate, boxQuantity, pieceQuantity];
}

/// Resets itemUpdateStatus back to idle after the screen has reacted to a
/// success/failure — mirrors OwnerQuotationUpdateResultConsumed above.
class OwnerQuotationItemUpdateResultConsumed extends OwnerQuotationEditEvent {
  const OwnerQuotationItemUpdateResultConsumed();
}