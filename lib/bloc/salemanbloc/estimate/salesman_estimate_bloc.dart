...
class SalesmanEstimateBloc extends Bloc<SalesmanEstimateEvent, SalesmanEstimateState> {
  ...
  SalesmanEstimateBloc({QuotationProvider? provider})
    ...
    on<QuotationSubmitRequested>(_onSubmitRequested);
    on<QuotationSubmitResultConsumed>(_onSubmitResultConsumed);
    on<QuotationItemUpdateSubmitted>(_onItemUpdateSubmitted);
    on<QuotationItemUpdateResultConsumed>(_onItemUpdateResultConsumed);
  }
  ...
  /// the Salesman Quotation Edit screen can react to it independently of
  /// the whole-quotation create/save/submit flow above.
  Future<void> _onItemUpdateSubmitted(
      QuotationItemUpdateSubmitted event,
      Emitter<SalesmanEstimateState> emit,
      ) async {
    emit(state.copyWith(
      itemUpdateStatus: ItemUpdateStatus.submitting,
    ...
    if (result.success) {
      emit(state.copyWith(
        itemUpdateStatus: ItemUpdateStatus.success,
        itemUpdateMessage: result.message,
      ));
    } else {
      emit(state.copyWith(
        itemUpdateStatus: ItemUpdateStatus.failure,
  ...
  void _onItemUpdateResultConsumed(
      QuotationItemUpdateResultConsumed event,
      Emitter<SalesmanEstimateState> emit,
      ) {
    emit(state.copyWith(
      itemUpdateStatus: ItemUpdateStatus.idle,
      clearItemUpdateMessage: true,
      clearItemUpdateError: true,
    ));
  }
